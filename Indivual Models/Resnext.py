...
def load_pkl(data_file):
    ...
    df_dataset.to_csv('csv/dataset.csv', index=False)
...
def predict(model,test_loader):
    ...
    df.to_csv("csv/test_predictions.csv", index=False)
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            ...
            torch.save({
                ...
                'optimizer_state_dict': optimizer.state_dict(),
                'best_acc': best_acc
            }, "best_weights/best_ResNEXT.pth")
            print("✅ Saved new BEST model!")
            # Save predictions to CSV (outside the if)
    
            pd.DataFrame({"prediction": all_preds}).to_csv("csv/preds.csv", index=False)
...
if __name__ == "__main__":
    ...
    X_tensor = torch.tensor(np.stack(train["mel"])   ).unsqueeze(1)
    ...
    X_tensor = torch.tensor(np.stack(val["mel"])   ).unsqueeze(1)
    ...
    X_tensor = torch.tensor(np.stack(test["mel"])   ).unsqueeze(1)
